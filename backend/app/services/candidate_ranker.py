...
class CandidateRanker:
    def rank(
        ...
        for idx, species in enumerate(species_catalog):
            ...
            if not dino_ref or all(x == 0.0 for x in dino_ref):
                ...
                text_vector = (
                    ...
                    if text_embedding
                    else (siglip_text_ref if siglip_text_ref else [0.0] * 8)
                )
            ...
            ranked.append(
                {
                    ...
                    "explanation": (
                        "Coincidencia visual orientativa, pero faltan rasgos diagnosticos."
                        if observation_representation.evidence_penalty
                        else "Coincidencia visual y contextual orientativa."
                    ),
        ...
        ranked.sort(
            key=lambda item: (item["risk_level"] in {"deadly", "high"}, item["confidence"]),
            reverse=True,
        )
        return ranked[:top_k]
    ...
    def _vector_similarity(self, left: list[float], right: list[float]) -> float:
        if not left or not right:
            return 0.0
        total = sum(abs(a - b) for a, b in zip(left, right, strict=False))
