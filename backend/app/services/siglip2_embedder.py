...
class SigLIP2Embedder(ImageTextEmbedder):
    ...
    def from_settings(cls, config: Settings) -> "SigLIP2Embedder":
        ...
        if is_real:
            try:
                ...
                model_identifier = config.siglip_model_path or config.siglip_model_name or "google/siglip-base-patch16-224"
                ...
                logger.warning(f"Failed to load real SigLIP2Embedder, falling back to mock: {e}")
                is_real = False
                device = "cpu"
    ...
    def embed_images(self, image_paths: list[str]) -> list[ImageEmbedding]:
        ...
        try:
            ...
            logger.warning(f"Error in real SigLIP2Embedder embed_images, falling back to mock: {e}")
    ...
    def embed_texts(self, texts: list[str]) -> list[TextEmbedding]:
        ...
        try:
            ...
            logger.warning(f"Error in real SigLIP2Embedder embed_texts, falling back to mock: {e}")
