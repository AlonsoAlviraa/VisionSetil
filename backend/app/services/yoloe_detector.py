...
class YOLOEDetector(MushroomDetector):
    ...
    def from_settings(cls, config: Settings) -> "YOLOEDetector":
        ...
        model = None
        model_name = config.yoloe_model_name or ""
        ...
        if is_real:
            try:
                ...
                # Determine model identifier: prefer explicit path, then explicit name.
                # Do NOT silently fall back to "yolov8n.pt" when the user set a path
                # that doesn't exist AND cleared the model name.
                ...
                if model_identifier is None:
                    raise FileNotFoundError(
                        f"No usable YOLO model: path='{model_path}' (exists={Path(model_path).exists() if model_path else False}), name='{model_name}'"
                    )
                ...
                try:
                    ...
                    if model_identifier is None:
                        raise FileNotFoundError(
                            "No model identifier available for cpu retry"
                        ) from e
                    model = YOLO(model_identifier)
                    device = "cpu"
                    ...
                    if not config.allow_mock_fallbacks:
                        raise RuntimeError(
                            f"YOLOEDetector real model failed to load (allow_mock_fallbacks is False): {e2}"
                        ) from e2
                    device = "cpu"
                    model = None
        ...
        if not is_real:
            if not config.allow_mock_fallbacks:
                raise RuntimeError(
                    "YOLOEDetector real model failed to load (allow_mock_fallbacks is False)."
                )
            logger.warning("YOLOE real model not available, using fallback detector")

        return cls(
            model_name=config.yoloe_model_name
            or ("yolov8n.pt" if config.use_real_yoloe else "mock-yoloe-26"),
