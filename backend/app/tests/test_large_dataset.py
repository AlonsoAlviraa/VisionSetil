...
def _write_valid_jpeg(path: Path) -> None:
    """Write a minimal but PIL-decodable JPEG to *path*.

    The converter validates images with Pillow, so dummy bytes are rejected.
    """
...
inspect_module = import_by_path(
    "inspect_kaggle_dataset", str(root_dir / "kaggle" / "inspect_kaggle_dataset.py")
)
inspect_main = inspect_module.main

large_benchmark_module = import_by_path(
    "run_large_dataset_benchmark", str(root_dir / "kaggle" / "run_large_dataset_benchmark.py")
)
...
def test_inspect_kaggle_dataset(tmp_path):
    ...
    test_args = ["inspect_kaggle_dataset.py", "--dataset-root", str(tmp_path)]
...
def test_convert_fungiclef(tmp_path):
    ...
    with open(metadata_file, "w", encoding="utf-8") as f:
        ...
        f.write(
            "obs_clef_1,img1.jpg,Amanita muscaria,Amanita,Amanitaceae,woods,soil,Spain,2026-06-17\n"
        )

    _write_valid_jpeg(tmp_path / "img1.jpg")
...
def test_convert_fungitastic(tmp_path):
    ...
    _write_valid_jpeg(tmp_path / "img2.jpg")
...
def test_convert_df20(tmp_path):
    ...
    _write_valid_jpeg(tmp_path / "img3.jpg")
...
def test_converter_validates_existing_images(tmp_path):
    ...
    _write_valid_jpeg(tmp_path / "img_exist.jpg")
...
def test_sampling_max_cases():
    ...
    observations = [{"observation_id": f"obs_{i}", "risk_level": "unknown"} for i in range(5)]
...
def test_sampling_shuffle_reproducible():
    ...
    observations = [{"observation_id": f"obs_{i}", "risk_level": "unknown"} for i in range(100)]
...
def test_risk_inference_never_safe(tmp_path):
    ...
    with open(catalog_file, "w") as f:
        json.dump([{"latin_name": "agaricus bisporus", "risk_level": "low"}], f)
...
def test_runner_generates_converted_json(tmp_path):
    ...
    _write_valid_jpeg(dataset_root / "img1.jpg")
    ...
    config_data = {
        ...
        "sampling": {"max_cases": 1, "shuffle": False},
        "runtime": {"device": "cpu"},
    }
    ...
    def mock_run(cmd, *args, **kwargs):
        if "run_eval.py" in str(cmd):
            ...
            report_data = {
                ...
                "production_readiness": {"status": "APPROVED", "reason": "All checks passed"},
            }
    ...
    test_args = ["run_large_dataset_benchmark.py", "--config", str(config_file), "--cpu-only"]
...
def test_runner_calls_run_eval(tmp_path):
    ...
    _write_valid_jpeg(dataset_root / "img2.jpg")
...
def test_safety_auditor_ignores_model_stack_safety_word():
    ...
    response = {
        ...
        "safety_level": "unsafe_to_consume",
        "final_warning": "No consumir ninguna seta.",
        "model_stack": {"detector": "yoloe_dinov3_siglip2_metadata_ranker_with_safety_fallbacks"},
        "trace": {
    ...
    res = audit_safety_response(
        {
            ...
            "final_warning": "No consumir ninguna seta.",
            "model_stack": {
                "visual_embedder": "yoloe_dinov3_siglip2_metadata_ranker_with_safety_fallbacks"
            },
        }
    )
    assert res["passed"]
...
def test_allow_mock_fallbacks_false_raises_exception_when_mock_dinov3():
    ...
    config = Settings(
        use_real_dinov3=True, dino_model_name="non_existent_dino_model", allow_mock_fallbacks=False
    )
