...
prepare_module = import_by_path("prepare_kaggle_dataset", str(root_dir / "kaggle" / "prepare_kaggle_dataset.py"))
prepare_main = prepare_module.main

benchmark_module = import_by_path("run_kaggle_benchmark", str(root_dir / "kaggle" / "run_kaggle_benchmark.py"))
...
def test_prepare_dataset_export(tmp_path, temp_dataset):
    ...
    with open(output_dir / "real_observations.json", "r", encoding="utf-8") as f:
...
def test_run_kaggle_benchmark_config_and_pipeline(tmp_path, temp_dataset):
    ...
    with open(output_dir / "model_status.json", "r") as f:
        status = json.load(f)
...
def test_run_kaggle_benchmark_max_cases(tmp_path, temp_dataset):
    ...
    with open(output_dir / "real_report.json", "r") as f:
        rep = json.load(f)
