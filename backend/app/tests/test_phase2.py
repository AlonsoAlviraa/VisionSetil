...
from app.ml.interfaces import ObservationRepresentation, MetadataVector
...
def test_open_set_rejection_deadly_lookalike():
    service = OpenSetRejectionService()
    candidates = [{"taxon": "Boletus edulis", "confidence": 0.8, "lookalikes": ["Amanita phalloides"]}]
    rep = ObservationRepresentation(
        vector=[],
        detected_views=["gills_or_pores", "base", "environment"],
        evidence_penalty=0.0,
...
def test_human_review_safe_to_eat_blocking(client: TestClient):
    ...
    rev_res = client.post(f"/observations/{obs_id}/request-human-review", json={"priority": "high", "reason": "test"})
    assert rev_res.status_code == 201
    rev_id = rev_res.json()["id"]
    
    update_res = client.patch(f"/human-reviews/{rev_id}", json={"reviewer_notes": "Esta seta es comestible"})
