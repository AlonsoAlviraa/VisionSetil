#!/usr/bin/env python3
"""Build E19 GBIF-mega notebook and push GPU kernel to Kaggle."""
...
K = REPO / "kaggle"
SLUG = "alonsoalviraaaa/visionsetil-exp-v19-gbif-mega"
OUT_LOCAL = K / "kernel_output_v19"
...
def build() -> Path:
    run([sys.executable, str(K / "build_exp_v19_gbif_mega.py")])
    nb = K / "visionsetil_exp_v19_gbif_mega.ipynb"
    if not nb.is_file():
...
def gbif_dataset_ready() -> bool:
    """True when Kaggle accepts ListDatasetFiles for the GBIF pack (post-processing done)."""
...
def push(nb: Path, wait_dataset: bool = True) -> None:
    ...
    meta = json.loads((K / "kernel-metadata-exp-v19.json").read_text(encoding="utf-8"))
    code = "visionsetil_exp_v19_gbif_mega.ipynb"
    meta["code_file"] = code
    meta["id"] = SLUG
    # Ensure GBIF dataset is attached
    ds = list(meta.get("dataset_sources") or [])
    if GBIF_DATASET not in ds:
        ds.append(GBIF_DATASET)
    if "picekl/fungitastic" not in ds:
        ds.insert(0, "picekl/fungitastic")
    ...
    if "not valid dataset sources" in out.lower():
        print(
            "FATAL: GBIF dataset source rejected — wait for processing then:\n"
            "  python scripts/push_kaggle_e19.py --push-only"
        )
    ...
    print(f"  python scripts/push_kaggle_e19.py --status")
    print(f"  python scripts/push_kaggle_e19.py --download")
