...
def convert_df20(dataset_root, output_json, poisonous_catalog_path=None, sampling_options=None):
    """
    Converts Danish Fungi 2020 (DF20) metadata to VisionSetil format.
    """
    ...
    print(f"DF20 Converter: Found metadata file at {metadata_file}")
    rows, columns = read_rows_from_file(metadata_file)
    if not rows:
        raise ValueError(f"No rows read from DF20 metadata file {metadata_file}")

    mapping = detect_column_heuristics(columns)
    print(f"DF20 Column Mapping Heuristics: {json.dumps(mapping, indent=2)}")
    ...
    for obs_id, group in obs_groups.items():
        ...
        metadata = {
            ...
            "user_notes": "Converted from DF20 / Danish Fungi 2020."
        }
    ...
    print(f"DF20 Converter: Converted and saved {len(final_sampled_list)} cases to {output_json}")
