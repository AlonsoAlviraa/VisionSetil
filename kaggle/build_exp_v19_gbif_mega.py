#!/usr/bin/env python3
"""
E19 GBIF MEGA — FungiTastic + GBIF ES allowlist40 (~38k images) multi-source train.

After E18 (FT + weak folder packs):
  - Mount alonsoalviraaaa/visionsetil-gbif-es-allowlist40
  - JSONL manifest loader + folder layout under images/<Species_safe>/
  - Anti-leak by observation_id (gbif_*)
  - fair_cap prefers cc_ok then includes NC research data (orientation only)
  - Hard-fail if GBIF mount exists but contributes 0 rows
  - Require fungitastic + gbif_es (optional soft folder packs)
...
Usage:
  python kaggle/build_exp_v19_gbif_mega.py
  python scripts/push_kaggle_e19.py
"""
...
# ── 1) Dataset detection: FT + GBIF + optional folder packs ───────────────────
...
new_detect_paths = """    FUNGICLEF_PATHS = [
    ...
    # E19: GBIF ES allowlist40 StillImage pack (~38k)
    GBIF_ES_PATHS = [
    ...
    # Optional soft folder packs (may contribute ~0 allowlist rows)
    MUSHROOM1_PATHS = [
...
new_fc_detect = """    # Try FungiCLEF / DF20 pack (optional)
    for p in FUNGICLEF_PATHS:
        if Path(p).exists():
            datasets['fungiclef'] = Path(p)
        ...
        if Path(p).exists():
            ...
            for sub in cand.iterdir() if cand.is_dir() else []:
                if sub.is_dir() and ('gbif' in sub.name.lower() or (sub / 'obs_gbif_es.jsonl').exists()):
                    ...
                    log(f"  ✓ Found gbif_es (nested): {sub}")
                    break
...
new_cell4 = (
    "# ═══ CELL 4: E19 multi-source loader (fungi_csv_loader + GBIF JSONL) ═══\n"
    "# FungiTastic + GBIF ES allowlist40; optional folder packs soft.\n"
    "\n"
    + loader_body
    + "\n\n"
    ...
    "    df = pd.DataFrame()\n"
    "else:\n"
    '    log(f"E19 source_db counts: {df[\'source_db\'].value_counts().to_dict()}")\n'
    "    _present = set(df['source_db'].unique().tolist()) if 'source_db' in df.columns else set()\n"
    "    # Hard-fail: GBIF mount present but 0 rows\n"
...
# ── 3) Allowlist filter (40 spp, caps 200/400, prefer_cc) ─────────────────────
...
new_sub = f"""# ═══ CELL 5: E19 GBIF-MEGA industrial allowlist (40 spp — NOT 80) ═══
...
REQUIRED_CORE = {{'fungitastic', 'gbif_es'}}
...
if len(df) > 0:
    ...
    # Hard-fail: GBIF must contribute after allowlist when mounted
    ...
    for sp, n in species_counts.items():
        # Deadly sparse (A. virosa 10, C. rubellus 1): allow min 1–2 obs
        mn = 1 if str(sp).lower() in DEADLY_FORCE else 3
        if n >= mn:
            keep.append(sp)
    ...
    # Caps: 200 normal / 400 deadly; fair per-source; prefer cc_ok then NC research
    MAX_OBS = 200
    MAX_OBS_DEADLY = 400
    ...
    if n_src3 < 2:
        ...
        log(f"  FATAL: {{msg}}")
        raise RuntimeError(msg)
    if 'gbif_es' not in src_counts3:
        raise RuntimeError(f"GBIF GATE: gbif_es dropped after caps: {{src_counts3}}")
    ...
    log(f"E19 gbif-mega: imgs={{len(df)}} spp={{df['species'].nunique()}} obs={{df['observation_id'].nunique()}}")
    ...
    raise RuntimeError(
        "MULTI-SOURCE GATE: empty df after allowlist — cannot train E19 GBIF mega"
    )
"""
...
# ── 4) Train hyperparams (E18/E17 carry-forward + E19 tag) ────────────────────
src = src.replace("epochs: int = 8", "epochs: int = 40  # E19")
src = src.replace("patience: int = 3", "patience: int = 10  # E19 dual early-stop")
src = src.replace("warmup_epochs: int = 1", "warmup_epochs: int = 2  # E19")
src = src.replace("swa_start_epoch: int = 6", "swa_start_epoch: int = 28  # E19")
src = src.replace("center_loss_weight: float = 0.01", "center_loss_weight: float = 0.05")
src = src.replace("batch_size: int = 16", "batch_size: int = 10  # E19 multi-view VRAM")

src = src.replace(
    "for epoch in range(start_epoch, cfg.epochs):\n    img_size = 224",
    "for epoch in range(start_epoch, cfg.epochs):\n"
    "    # E19: fixed 224 (P100-safe)\n"
    "    img_size = 224",
)
...
log(f"E19 deadly class_weights x12 n={len(deadly_label_indices)}")
"""
...
src = src.replace(
    ...
    "            # E19: push deadly true class into top-3\n"
...
src = src.replace(
    ...
    "        'experiment': 'E19-gbif-mega', 'allowlist': 'industrial_v1',\n"
    "        'epochs': 40,\n"
    "        'sources_planned': ['fungitastic', 'gbif_es'],\n"
    "        'sources_optional': ['mushroom1', 'combined_mushrooms', 'mush215'],\n"
    "        'gbif_images_approx': 38003,\n"
    "        'baseline_e17_map': 0.194, 'baseline_e17_deadly': 0.447,\n"
    "        'gate_map': 0.22, 'gate_deadly': 0.50,\n"
    "        'soft_gate_map': 0.25, 'soft_gate_deadly': 0.90,\n"
    "        'prefer_cc_ok': True,\n"
    "        'note': 'do not expand to 80 spp until MAP@3>=0.22 AND deadly@3>=0.50; orientation only',\n"
    "    },\n"
    "    'deadly_species_known': len(DEADLY_SPECIES),\n"
    "    'deadly_species_in_dataset': len(deadly_label_indices),\n"
    "    'version': 'v19-E19-gbif-mega',\n"
    "    'attribution': 'FungiTastic (Picek et al.) + GBIF ES StillImage; educational orientation only',",
)
...
_new_do = (
    ...
    'log(f"  DO7b: GBIF in used ........... {\'✅\' if \'gbif_es\' in _dbs else \'❌\'}")\n'
    'log(f"  DO8: Artifacts exported ...... ✅")\n'
    'log(f"  DO9: ECE < 0.15 .............. {\'✅\' if ece < 0.15 else \'⚠️\'} ({ece:.4f})")\n'
    'log(f"  DO10: Per-species diag ....... ✅")\n'
    'log("  Scope: allowlist stays 40 until expand-to-80 gates pass; orientation only")'
)
...
src = src.replace("TRAINING COMPLETE! (v8)", "TRAINING COMPLETE! (v19-E19-gbif-mega)")
src = src.replace(
    'out_path = Path(__file__).parent / "visionsetil_mega_training.ipynb"',
    'out_path = Path(__file__).parent / "visionsetil_exp_v19_gbif_mega.ipynb"',
)
src = src.replace(
    "VisionSetil Multi-View Mega Training v8",
    "VisionSetil E19 GBIF-MEGA — FT + GBIF ES allowlist40",
)
src = src.replace(
    "VisionSetil v8 — Multi-View SOTA Training (3x BUG-FIXED)",
    "VisionSetil E19 GBIF-MEGA — multi-source + ~38k GBIF images",
)

ns = {"__name__": "__main__", "__file__": str(ROOT / "gen_notebook_v8.py")}
try:
    exec(compile(src, "gen_e19.py", "exec"), ns)
except Exception as e:
    print("EXEC FAILED:", e)
    raise

out = ROOT / "visionsetil_exp_v19_gbif_mega.ipynb"
if not out.is_file():
    raise SystemExit(f"MISSING notebook {out}")
# Build-time landmine guard: '\\' path replace in ipynb JSON becomes SyntaxError on Kaggle
